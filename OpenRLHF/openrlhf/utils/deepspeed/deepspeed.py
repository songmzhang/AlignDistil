...
class DeepspeedStrategy(ABC):
    ...
    def get_ds_eval_config(self, offload=False, stage=None):
        # DS Config
        if hasattr(self.args, "static_model_stage") and self.args.static_model_stage == 3:
            ds_config = get_eval_ds_config(offload=offload, stage=3, bf16=self.bf16)
        elif stage is not None:
            ds_config = get_eval_ds_config(offload=offload, stage=stage if stage == 3 else 0, bf16=self.bf16)
        else:
