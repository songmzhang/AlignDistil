...
class SFTDataset(Dataset):
    ...
    def __init__(
        ...
        ori_dataset_len = len(processed_dataset["prompt"])
        processed_dataset = processed_dataset.filter(lambda x: x["prompt"] is not None)
        if self.strategy.is_rank_0():
            cur_dataset_len = (ori_dataset_len - len(processed_dataset["prompt"])) / ori_dataset_len
            logger.info(f'{ori_dataset_len - len(processed_dataset["prompt"])} in {ori_dataset_len} ({cur_dataset_len * 100:.4f}%) samples have longer prompt than max_length {self.max_length} and are dropped.')
        # print(processed_dataset, ori_dataset_len)

        # print(processed_dataset)
        # sadf
        
        if not self.pretrain_mode:
            text = (processed_dataset["prompt"][0] + processed_dataset["response"][0]).rstrip("\n")
            if not text.endswith(self.tokenizer.eos_token):
                text += " " + self.tokenizer.eos_token
        else:
            text = processed_dataset["prompt"][0]
        
        self.strategy.print("Data Example:\n{}".format(text))
        # self.strategy.print("response:\n{}".format(processed_dataset["response"][0]))
