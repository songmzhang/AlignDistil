...
class KDExperience:
    ...
    def to_device(self, device: torch.device) -> None:
        self.sequences = to(self.sequences, device)
        self.teacher_logits = to(self.action_log_probs, device)
        if self.reference_logits is not None:
            self.reference_logits = to(self.reference_logits, device)
        if self.attention_mask is not None:
            self.attention_mask = self.attention_mask.to(device)
        if self.action_mask is not None:
            self.action_mask = self.action_mask.to(device)
    ...
    def pin_memory(self):
        self.sequences = pin_memory(self.sequences)
        self.teacher_logits = pin_memory(self.teacher_logits)
        if self.reference_logits is not None:
            self.reference_logits = pin_memory(self.reference_logits)
        if self.attention_mask is not None:
            self.attention_mask = self.attention_mask.pin_memory()
        if self.action_mask is not None:
            self.action_mask = self.action_mask.pin_memory()
...
class NaiveExperienceMaker(ABC):
    ...
    def make_experience_list(self, all_prompts: Union[str, List[str]], **generate_kwargs) -> List[Experience]:
        ...
        for experience in experiences:
            ...
            if experience.info["reward"].dim() > 1:  # token to sequence reward
    ...
    def make_experience(self, samples: Samples) -> Experience:
        ...
        if not self.remote_rm_url:
            for rm in self.reward_model:
                if self.strategy.args.token_level_dpo_reward:
                    r_refs.append(rm.forward.remote(sequences_cpu, num_actions, attention_mask_cpu, packed_seq_lens=packed_seq_lens))
                else:
        ...
        if r.dim() == 2:   # token-level reward
            dpo_policy_log_probs = r
            r = (dpo_policy_log_probs - base_action_log_probs) * self.strategy.args.beta
...
class RemoteKDExperienceMaker(RemoteExperienceMaker):
    def __init__(self, *args, teacher_model, ref_model=None, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def make_experience(self, samples: Samples) -> Experience:
        ...
        # init log probs
        reference_logits_ref = self.ref_model.forward.remote(
            sequences_cpu, attention_mask=attention_mask_cpu, return_output=True
        )

        # values
        teacher_logits_ref = self.teacher_model.forward.remote(
            sequences_cpu, attention_mask=attention_mask_cpu, return_output=True
        )

        # wait initial/critic/reward model done
        start = time.time()
        ref_values = ray.get([reference_logits_ref, teacher_logits_ref])
        wait_time = time.time() - start

        reference_logits, teacher_logits = ref_values[0], ref_values[1]
        teacher_logits = teacher_logits[:, -num_actions:, :]
        reference_logits = reference_logits[:, -num_actions:, :]
        reference_logits = reference_logits.to(device)
