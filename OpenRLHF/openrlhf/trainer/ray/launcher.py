...
@ray.remote(num_gpus=1)
class TeacherModelRayActor(BasePPORole):
    def init_model_from_pretrained(self, strategy: DeepspeedStrategy, pretrain):
        self._setup_distributed(strategy)
        model = Actor(
            pretrain,
            ...
            bf16=strategy.args.bf16,
            load_in_4bit=strategy.args.load_in_4bit,
            ds_config=strategy.get_ds_eval_config(offload=strategy.args.teacher_offload),
            packing_samples=strategy.args.packing_samples,
        )
        strategy.print(model)

        if strategy.args.teacher_offload:
            model._offload = True
...
class KDRayActorGroup(PPORayActorGroup):
    """
    A group of ray actors for knowledge distillation
    Functions start with 'async' should return list of object refs
    ...
    def async_fit_kd(
        self,
        teacher_model_group: "KDRayActorGroup",
        ref_model_group: "KDRayActorGroup" = None,
        vllm_engines: List = None,
    ):
        """Conduct knowledge distillation.

        Args:
            teacher_model_group (KDRayActorGroup): teacher model group.
            ref_model_group (KDRayActorGroup): reference model group.
            vllm_engines: vllm engines for text generation, if not specified, generate text by actor model directly.

        Returns:
            List: list of remote object refs.
        """
        teacher_actors = teacher_model_group._actor_handlers
        if ref_model_group is not None:
            ref_actors = ref_model_group._actor_handlers

        refs = []
        # TODO(wuxibin): actor model choose critic/reward/initial model in a
        # round robin fashion, implement more efficient dispatching strategy.
        for i, actor in enumerate(self._actor_handlers):
            teacher_actor = teacher_actors[i % len(teacher_actors)]
            if ref_model_group is not None:
                ref_actor = ref_actors[i % len(ref_actors)]
            else:
                ref_actor = None

            refs.append(
                actor.fit.remote(
                    teacher_model=teacher_actor,
                    ref_model=ref_actor,
                    vllm_engines=vllm_engines,
                )
            )
