...
class TDPOTrainer(ABC):
    """
    Trainer for Token-level Direct Preference Optimization (DPO) training.
    ...
    def fit(self, args, consumed_samples=0, num_update_steps_per_epoch=None):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                ...
                c_loss_masks = loss_masks[: chosen_ids.shape[0]]
                r_loss_masks = loss_masks[chosen_ids.shape[0]:]
                chosen_logp = F.log_softmax(chosen_logits, dim=-1, dtype=torch.float32)
                reference_chosen_p = F.softmax(reference_chosen_logits, dim=-1, dtype=torch.float32)
                chosen_fkl = (reference_chosen_p * (reference_chosen_p.log() - chosen_logp)).sum(-1)
                chosen_fkl = (chosen_fkl[:, 1:] * c_loss_masks).sum(-1)
                
                rejected_logp = F.log_softmax(rejected_logits, dim=-1, dtype=torch.float32)
                reference_rejected_p = F.softmax(reference_rejected_logits, dim=-1, dtype=torch.float32)
                rejected_fkl = (reference_rejected_p * (reference_rejected_p.log() - rejected_logp)).sum(-1)
                rejected_fkl = (rejected_fkl[:, 1:] * r_loss_masks).sum(-1)
                ...
                logs_dict = {
                    ...
                    "reference_chosen_logps": reference_chosen_logps.mean().item(),
                    "reference_rejected_logps": reference_rejected_logps.mean().item(),
    ...
    def concatenated_forward(self, model, chosen_ids, c_mask, reject_ids, r_mask, prompt_id_lens):
        ...
        all_logps_sum, all_logps_mean, loss_masks = self._get_batch_logps(
        ...
        chosen_logits = all_logits[: chosen_ids.shape[0]]
        rejected_logits = all_logits[chosen_ids.shape[0] :]
        aux_loss = output.aux_loss if "aux_loss" in output else []
        return chosen_logps, rejected_logps, chosen_logits, rejected_logits, loss_masks, aux_loss, -all_logps_mean[: chosen_ids.shape[0]].mean()
    ...
    def _get_batch_logps(
        ...
        return logprobs_sums, logprobs_means, loss_masks
